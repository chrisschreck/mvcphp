...
<div class="grid-x grid-margin-x">
    ...
    <div class="grid-x grid-margin-x">
        <div class="cell large-8">
            <div class="content">
                <form method="POST">
                    <label class="form-label required">Name des Bildschirmes:</label>
                    <input type="text" name="screen_name" placeholder="Screenname" value="<?php echo $screen->getName()?>" required>
                    <label class="form-label required">Status des Bildschirmes:</label>
                    <input type="checkbox" name="screen_online" <?php echo ($screen->getOnline() == 1 ? 'checked' : '' ); ?> value="1"> - Bildschirm online schalten<br><br>
                    ...
                    <?php
                        ...
                        if(isset($data['selected_packages']['slot-0'])){
                            foreach($selected_packages['slot-0'] as $key => $value){
                                $inputs .= "<input type='hidden' name='screen[slot-0][]' value='".$value->getId()."'>";
                                echo '<div class="drag-element" data-name="screen[slot-0]" id="'.$value->getId().'" value="'.$value->getId().'" draggable="true" ondragstart="drag(event)" width="336" height="69" selected>'.$value->getName().'</div>';
                            }
                        }
                        echo '</div> </div>';
                        if($screen->getMode() == "four_divided") {
                            ...
                            if(isset($data['selected_packages']['slot-1'])){
                                foreach($selected_packages['slot-1'] as $key => $value){
                                    $inputs .= "<input type='hidden' name='screen[slot-1][]' value='".$value->getId()."'>";
                            ...
                            if(isset($data['selected_packages']['slot-2'])){
                                foreach($selected_packages['slot-2'] as $key => $value){
                                    $inputs .= "<input type='hidden' name='screen[slot-2][]' value='".$value->getId()."'>";
                            ...
                            if(isset($data['selected_packages']['slot-3'])){
                                foreach($selected_packages['slot-3'] as $key => $value){
                                    ...
                                    $inputs .= "<input type='hidden' name='screen[slot-3][]' value='".$value->getId()."'>";
                                }
                            }
                            echo ' </div></div>';
                        }
                    ...
                    <div id="inputs">
                        <?php if(isset($inputs) && !empty($inputs)) {echo $inputs;}?>
                    </div>
                </form>
            </div>
        </div>
